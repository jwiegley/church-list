...
newtype Concat a = Concat (forall r. Monoid r => (a -> r) -> r)

instance Show a => Show (Concat a) where
    show (Concat c) = show (c (:[]))

instance Functor Concat where
    fmap f (Concat c) = Concat (\k -> c (k . f))

instance Foldable Concat where
    foldMap f (Concat c) = c f

instance Traversable Concat where
    traverse f (Concat c) =
...
instance Applicative Concat where
    pure x = Concat (\k -> k x)
    Concat f <*> Concat x = Concat (\k -> f (\g -> x (k .g)))
...
instance Monoid (Concat a) where
    mempty  = Concat $ const mempty
    Concat x `mappend` Concat y = Concat $ \k -> x k `mappend` y k
...
lastMay (Concat c) = getLast $ c (Last . Just)

null :: Concat a -> Bool
null = isNothing . headMay

length :: Concat a -> Int
length (Concat c) = getSum $ c (const (Sum 1))
