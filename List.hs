...
prop_uncons xs = uncons xs == L.uncons xs
...
scanl f q = fromList . L.scanr f q . F.toList

scanr1 :: (a -> a -> a) -> List a -> List a
scanr1 = caseList mempty (scanr f)

mapAccumL :: (acc -> x -> (acc, y)) -> acc -> List x -> (acc, List y)
mapAccumL f q = fromList . L.mapAccumL f q . F.toList

mapAccumR :: (acc -> x -> (acc, y)) -> acc -> List x -> (acc, List y)
mapAccumL f q = fromList . L.mapAccumR f q . F.toList

iterate :: (a -> a) -> a -> List a
iterate f a = List $ \k -> k a <> iterate f (f a)

repeat :: a -> List a
repeat a = List $ \k -> k a <> repeat a

replicate :: Int -> a -> List a
replicate n a
    | n <= 0    = mempty
    | otherwise = List $ \k -> k a <> replicate (n-1) a

cycle :: List a -> List a
cycle xs = xs <> cycle xs

unfoldr :: (b -> Maybe (a, b)) -> b -> List a
unfoldr f b = List $ \k -> case f b of
    Nothing -> mempty
    Just (a, b') -> k a <> unfoldr f b'
...
dropWhileEnd p = fromList . L.dropWhileEnd f xs . F.toList
...
lookup a (Concat c) = getFirst $ c $ \(x, b) ->
    First (mfilter (const (a == x)) (Just b))

filter :: (a -> Bool) -> List a -> List a
filter f (Concat c) = c $ \a -> mfilter f (pure a)

partition :: (a -> Bool) -> List a -> (List a, List a)
partition f (Concat c) = c $ \a ->
...
groupBy f = fromList . map fromList . L.groupBy f . F.toList
...
genericReplicate n a
    | n <= 0    = mempty
    | otherwise = List $ \k -> k a <> genericReplicate (n-1) a
